...
function init() {
   ...
   function startGame() {
      // Fix bug: When the user swaps the blank but does not click the 
      // start button, the user will not be able to move the position 
      // of the last blank after clicking the start button.
      for (let items of blocks) {
         items.onclick = swapWithBlank;
      }

      // Fix bug: The value of the timer is not correct when the user
      // repeatedly clicks the start button.
      ...
      // Fix bug: Can not "slide" - one of the numbers appears twice.
      scrambledOrder[8] = 0;
      for (let i = 1; i <= 9; i++) {
         let currentBlock = blockContainer.children[i-1];
         currentBlock.id = "block" + scrambledOrder[i-1];
         currentBlock.firstElementChild.src = "image/block" + scrambledOrder[i-1] + ".jpg";
         
         // Fix bug: The alt of the image is different from the id of the same image.
         currentBlock.firstElementChild.alt = scrambledOrder[i-1];
      }  
