...
function init() {
   ...
   // Fix bug - 
   // remove semicolon in the fetch statement
   // use a fat arrow instead of greater than symbol
   fetch("authorlist.json")
      ...
      .then (json => {
         ...
         // Fix bug - 
         // Use a question mark in the query string
         // call the encodeURIComponent() to format the query string
         // use response.text() to parse the text context
         selectionList.onchange = function() {
            fetch("sfreviews.pl?author=" + encodeURIComponent(selectionList.value))
            .then ( response => {
               if (response.ok) {
                  return response.text();
               } else {
                  return "Unable to retrieve author list";
               }
            })
   ...
   // Fix bug -
   // Use the DOMParser() constructor instead of Parser()
   // Use two backtick symbols instead of two quotation marks
   fetch("sfpod.xml")
